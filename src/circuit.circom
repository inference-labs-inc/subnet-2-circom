...
template IncentiveMechanism(batch_size, bits){
    ...
    signal input COMPETITION_WEIGHT;
    ...
    signal input competition[batch_size];
    ...
    signal competition_weighted[batch_size];
    ...
    for (var i=0; i<batch_size; i++) {
        ...
        temp_3[i] <== competition[i] * COMPETITION_WEIGHT;
        int_div_3[i] = IntDiv(bits);
        int_div_3[i].in[0] <== temp_3[i];
        int_div_3[i].in[1] <== scaling;
        competition_weighted[i] <== int_div_3[i].out;
        ...
        calculated_score_fraction_comp[i].competition_weighted <== competition_weighted[i];
